...
library(circlize)
library(RColorBrewer)  #for obtaining colors

#---Figure 5A starts
# The p values for the sliding windows for all prioritized 33 TFs, based on both first and second peaks
...
myfacet_label_5a = c('first peak (23 TFs, FDR = 0.05)','second peak (13 TFs, FDR = 0.05)')
...
#---Figure 5C start, logically it makes sense to code 5C followed by 5B
...
# first peak times, second peak times of the 1939 genes
...
for (mytf in tf_union){
  ...
  # TF binding site of the 1939 genes (including both NA and non-NA peaks) 
...
for (i in genes_union_upp){
  if (grepl('HSF', i)){
    myrowname = 'red'
  }else if(grepl('ANAC',i) | grepl('NAP', i)){
    myrowname = 'blue'
...
ht1 = Heatmap(-log10(p_mat), col = col,cluster_columns = FALSE, 
              ...
              heatmap_width = unit(16.5, "cm"), cluster_rows = mhclust,
...
# get the time scale
# This is window index - peak time matching
myat_first = c()
for (ii in c(4,5,6,7,8)){
  myat_first = c(myat_first, which.min(abs(ii - time_first)))
}
...
# a bit complicated, for different scales of first and second peaks
# This is a trick
...
#prepare HSF data
# _1 indicates the first peaks, _2 the second peaks
...
grid.text('B', x = unit(3,'mm'),y = unit(345,'mm'),gp = gpar(fontface = 'bold'))
grid.text('C', x = unit(3,'mm'),y = unit(190,'mm'),gp = gpar(fontface = 'bold'))
dev.off()
