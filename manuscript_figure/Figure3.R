...
# BIG NOTE: My advisor Fumi revised this script and we use Figure3_FK.R for figure 3
# This script is to generate figure 3 for the manuscript
...
#take a look at the ACP specific gene peak time in Daisuke data
#begin
load("peak.time.level.EdAvrRpt2.RData")
mean(peak.time[gene_group1], na.rm =T)
sd(peak.time[gene_group1], na.rm =T)
mean(peak.time)
sd(peak.time)
#end
...
col_flg22 = colorRamp2(c(-3,-2,-1, 0, 2 ,2.5,4,7), c("dodgerblue2","cornflowerblue",'lightskyblue1','white','lightsalmon','orange','orangered',"red4"))
col_flg22 = colorRamp2(c(-3,-2.5,-1, 0, 2 ,2.5,4,7), c("dodgerblue2","cornflowerblue",'lightskyblue1','white','white','orange','orangered',"red4"))

bot_anno = HeatmapAnnotation(foo = anno_text(colnames(flg22_mat1[,1:6]), rot = 0, 
                                             just = 'center',gp = gpar(fontsize = 11), location = unit(1,'mm')),
                             show_annotation_name = F)
flg22_ht1 = Heatmap(flg22_mat1[,1:6], cluster_rows = F, col = col_flg22,
              ...
              width = unit(60,'mm'), height = unit(length(gene_name_pt_group1)/16, 'mm'),
              show_heatmap_legend = F)
flg22_ht2 = Heatmap(flg22_mat2[,1:6], cluster_rows = F, col = col_flg22,
                    name = 'flg22_2',
                    cluster_columns = FALSE,show_row_names = F, 
                    column_title_gp = gpar(fontsize=14), column_names_gp = gpar(fontface = 'bold'),
                    border = "gray20", column_names_rot = 0,
                    na_col = "gray90", use_raster = F,
                    width = unit(60,'mm'), height = unit(length(gene_name_pt_group2)/16, 'mm'),
                    show_heatmap_legend = F)
flg22_ht3 = Heatmap(flg22_mat3[,1:6], cluster_rows = F, col = col_flg22,
                    name = 'flg22_3',
                    cluster_columns = FALSE,show_row_names = F, 
                    column_title_gp = gpar(fontsize=14), column_names_gp = gpar(fontface = 'bold'),
                    border = "gray20", column_names_rot = 0,
                    na_col = "gray90", use_raster = F,
                    width = unit(60,'mm'), height = unit(length(gene_name_pt_group3)/16, 'mm'),
                    show_heatmap_legend = F)

flg22_ht4 = Heatmap(flg22_mat4[,1:6], cluster_rows = F, col = col_flg22,
                    name = 'flg22_4', show_column_names = F,
                    cluster_columns = FALSE, show_row_names = F, 
                    column_title_gp = gpar(fontsize=14),
                    border = "gray20", column_names_rot = 0,
                    na_col = "gray90", use_raster = F,
                    width = unit(60,'mm'), height = unit(length(gene_name_pt_group4)/16, 'mm'),
...
jpeg("figures/Figure3.jpeg",height = 150, width = 250, units = "mm",res = 300)
